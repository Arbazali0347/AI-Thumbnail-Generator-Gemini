...
import { ChevronDownIcon, CpuIcon, ImageIcon, PenIcon, SparkleIcon, SquareIcon, type Sparkle } from "lucide-react";
...
const StyleSelector = ({ value, onChange, isOpen, setIsOpen }: { value: ThumbnailStyle; onChange: (style: ThumbnailStyle) => void; isOpen: boolean; setIsOpen: (open: boolean) => void }) => {
    ...
    const styleDescriptions: Record<ThumbnailStyle, string> = {
        "Bold & Graphic": "High contrast bold typography, striking virsuals",
        "Minimalist": "Clean design, ample white space, simple typography",
        "Photorealistic": "Photo-based, natural looking",
        "Illustrated": "Hand-drawn, creative, artistic",
        "Tech/Futuristic": "Sleek design, modern fonts, tech elements",
    }
    ...
    const styleIcons: Record<ThumbnailStyle, React.ReactNode> = {
        ...
        "Illustrated": <PenIcon className="h-4 w-4" />,
    ...
    return (
        <div className="relative space-y-3 dark ">
            ...
            <button type="button" onClick={() => setIsOpen(!isOpen)} className="flex w-full items-center justify-between rounded-md border px-4 py-3 text-left transition bg-white/8 border-white/10 text-zinc-200">
                    ...
                    {thumbnailStyles.map((style)=> (
                        <button key={style} type="button" onClick={()=> {onChange(style); setIsOpen(false)}} className="flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30">
                            ...
                            <div>
                                ...
                                <p className="text-xm text-zinc-400">{styleDescriptions[style]}</p>
                            </div>
                        </button>
                    ))}
                </div>
